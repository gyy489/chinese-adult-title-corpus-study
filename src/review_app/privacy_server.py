"""Serve the local-only residual-privacy human-review desk."""
...
from .privacy_store import DATA, MANIFEST, QUEUE, PrivacyReviewStore

STATIC = Path(__file__).with_name("privacy_static")


class Handler(BaseHTTPRequestHandler):
    store: PrivacyReviewStore
    token = ""
...
def main() -> None:
    ...
    parser.add_argument("--reviewer-id", default="researcher_privacy_reviewer_1")
    parser.add_argument("--port", type=int, default=8770)
    parser.add_argument(
        "--db", type=Path, default=DATA / "private/review_researcher.sqlite3"
    )
    ...
    print(f"Residual privacy review: {url}")
